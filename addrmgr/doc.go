...
nodes will connect and disconnect as they please. Each node must manage a source
of IP addresses to connect to and share with other nodes. The Decred protocol
provides the `getaddr` and `addr` messages, allowing peers to share known
addresses with each other. Each node also needs a way to store those addresses
and select peers from them. However, it is important to remember that remote
peers cannot be trusted. A remote peer might send invalid addresses, or worse,
only send addresses they control with malicious intent.
...
generally helps provide greater peer diversity. More importantly, it drastically
...
the selection toward known good peers. The general idea is to make a best effort
to only provide usable addresses.
*/
package addrmgr
